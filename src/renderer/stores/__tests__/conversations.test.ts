...
describe('useConversationsStore', () => {
  ...
  describe('SDK session ID management', () => {
    ...
    // =========================================================================
    // Bug fix: clearSdkSessionId must persist the cleared state to disk
    // =========================================================================
    describe('clearSdkSessionId persists cleared state to disk (Bug 1 fix)', () => {
      it('should save a payload WITHOUT sdkSessionId after clearSdkSessionId', async () => {
        // Arrange: conversation with messages and a session ID
        store.currentConversationId = 'conv_clear_persist';
        chatStore.setCurrentConversation('conv_clear_persist');
        chatStore.addUserMessage('Hello');
        store.setSdkSessionId('conv_clear_persist', 'stale-session-to-clear');

        // Act: clear the session ID then save
        store.clearSdkSessionId('conv_clear_persist');
        await store.saveCurrentConversation();

        // Assert: the saved payload must not carry the stale session ID
        expect(mockElectron.conversation.save).toHaveBeenCalledOnce();
        const savedPayload = mockElectron.conversation.save.mock.calls[0][0];
        expect(savedPayload.sdkSessionId).toBeUndefined();
      });

      it('should not restore a cleared session ID when loading the saved payload back', async () => {
        // Arrange
        store.currentConversationId = 'conv_no_restore';
        chatStore.setCurrentConversation('conv_no_restore');
        chatStore.addUserMessage('Test');
        store.setSdkSessionId('conv_no_restore', 'session-to-purge');

        // Act: clear then save
        store.clearSdkSessionId('conv_no_restore');
        await store.saveCurrentConversation();

        // Capture what was written to disk
        const persistedPayload = mockElectron.conversation.save.mock.calls[0][0];
        // Confirm the written payload has no session ID (precondition for the next step)
        expect(persistedPayload.sdkSessionId).toBeUndefined();

        // Simulate reloading from the persisted payload (e.g. app restart)
        mockElectron.conversation.get.mockResolvedValue(persistedPayload);
        await store.loadConversation('conv_no_restore');

        // The in-memory map must also have no session ID for this conversation
        expect(store.getSdkSessionId('conv_no_restore')).toBeUndefined();
      });

      it('should not affect other conversations when clearing one session ID', async () => {
        // Arrange: two conversations, each with a session ID and messages
        store.setSdkSessionId('conv_a', 'session-a');
        store.setSdkSessionId('conv_b', 'session-b');

        store.currentConversationId = 'conv_b';
        chatStore.setCurrentConversation('conv_b');
        chatStore.addUserMessage('From conv B');

        // Act: clear only conv_b's session and save
        store.clearSdkSessionId('conv_b');
        await store.saveCurrentConversation();

        const savedPayload = mockElectron.conversation.save.mock.calls[0][0];
        expect(savedPayload.sdkSessionId).toBeUndefined();

        // conv_a's session must be untouched
        expect(store.getSdkSessionId('conv_a')).toBe('session-a');
      });
    });
  });
});
